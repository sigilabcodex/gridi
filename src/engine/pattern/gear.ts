...
function ringLength(baseLength: number, div: number, ring: number) {
  ...
  const scale = 0.65 + ((Math.max(1, Math.min(8, div | 0)) - 1) / 7) * 1.25;
  if (ring === 0) return base;
  if (ring === 1) return Math.max(3, Math.min(MAX_STEPS, Math.round(base * (0.72 + scale * 0.5))));
  return Math.max(3, Math.min(MAX_STEPS, Math.round(base * (1.18 + (2 - scale) * 0.35))));
}
...
export function createGearModel(trigger: TriggerModule, voiceId: string): GearModel {
  const ringCount = gearRingCount(trigger.density);
  const rings: GearRingModel[] = [];
  for (let ring = 0; ring < 3; ring++) {
    const length = ringLength(trigger.length, trigger.subdiv, ring);
    const density = clamp01(0.24 + trigger.gravity * 0.38 + (ring === 0 ? 0.22 : ring === 1 ? 0.1 : 0.03));
    const pattern = generateRingPattern(length, trigger.seed ^ (ring * 0x45d9f3b), voiceId, density);
    const rotation = trigger.euclidRot + Math.round((ring - 1) * (2 + trigger.subdiv * 0.7));
    const phase = clamp01(trigger.weird) * (ring + 1) * 0.37;
    const weight = clamp01(0.45 + trigger.gravity * 0.4 - ring * 0.08);
    rings.push({ length, rotation, phase, weight, pattern });
  }
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const idxA = ((Math.floor(step + a.rotation + step * a.phase * drift) % a.length) + a.length) % a.length;
    const idxB = ((Math.floor(step + b.rotation + step * b.phase * drift * 1.2) % b.length) + b.length) % b.length;
    const idxC = ((Math.floor(step + c.rotation + step * c.phase * drift * 1.35) % c.length) + c.length) % c.length;
    ...
    const bChance = clamp01(strictness * 0.62 + b.weight * 0.34);
    const cLift = gateC ? 0.25 + c.weight * 0.45 : -0.08;
    const finalChance = clamp01(bChance + cLift - trigger.drop * 0.45);
